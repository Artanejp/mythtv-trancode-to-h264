...
if [ $FFMPEG_ENC -ne 0 ]; then
	...
	if [ ${TARGET_BITRATE_KBIT} -gt 0 ] ; then
	    # See: SvtAv1Enc --help .
	    typeset -i __DEFAULT_UNDERSHOOT
	    __DEFAULT_UNDERSHOOT=25
	    typeset -i __DEFAULT_OVERSHOOT
	    __DEFAULT_OVERSHOOT=25
	    typeset -i __DEFAULT_MAX_OVERSHOOT
	    __DEFAULT_MAX_OVERSHOOT=50
	    if [ ${SVTAV1_QUANT_UNDERSHOOT_PERCENT} -lt 0 ] ; then
		SVTAV1_QUANT_UNDERSHOOT_PERCENT=${__DEFAULT_UNDERSHOOT}
	    elif [ ${SVTAV1_QUANT_UNDERSHOOT_PERCENT} -gt 100 ] ; then
		SVTAV1_QUANT_UNDERSHOOT_PERCENT=${__DEFAULT_UNDERSHOOT}
	    fi
	    if [ ${SVTAV1_QUANT_OVERSHOOT_PERCENT} -lt 0 ] ; then
		SVTAV1_QUANT_OVERSHOOT_PERCENT=${__DEFAULT_OVERSHOOT}
	    elif [ ${SVTAV1_QUANT_OVERSHOOT_PERCENT} -gt 100 ] ; then
		SVTAV1_QUANT_OVERSHOOT_PERCENT=${__DEFAULT_OVERSHOOT}
	    fi
	    if [ ${SVTAV1_QUANT_MAX_OVERSHOOT_PERCENT} -lt 0 ] ; then
		SVTAV1_QUANT_MAX_OVERSHOOT_PERCENT=${__DEFAULT_MAX_OVERSHOOT}
	    elif [ ${SVTAV1_QUANT_MAX_OVERSHOOT_PERCENT} -gt 100 ] ; then
		SVTAV1_QUANT_MAX_OVERSHOOT_PERCENT=${__DEFAULT_MAX_OVERSHOOT}
	    fi
	    if [ ${IS_CRF} -eq 0 ] ; then
		__VCODEC_PARAMS="${__VCODEC_PARAMS}:tbr=${TARGET_BITRATE_KBIT}"
		__VCODEC_PARAMS="${__VCODEC_PARAMS}:undershoot-pct=${SVTAV1_QUANT_UNDERSHOOT_PERCENT}"
		__VCODEC_PARAMS="${__VCODEC_PARAMS}:overshoot-pct=${SVTAV1_QUANT_OVERSHOOT_PERCENT}"
		...
		__VCODEC_PARAMS="${__VCODEC_PARAMS}:undershoot-pct=${SVTAV1_QUANT_UNDERSHOOT_PERCENT}"
		__VCODEC_PARAMS="${__VCODEC_PARAMS}:mbr-overshoot-pct=${SVTAV1_QUANT_MAX_OVERSHOOT_PERCENT}"
	...
	__SVTAV1_CODEC_QPVAL=""
	    ...
	    __SVTAV1_CODEC_QPVAL="min-qp=${__CRF_MIN}"
	fi
	    ...
	    __SVTAV1_CODEC_QPVAL="${__SVTAV1_CODEC_QPVAL}max-qp=${__CRF_MAX}"
	fi
